...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    try {
      ...
      const { expireTime, ...rest } = Object.fromEntries(Object.entries(fields).map(([key, value]) => [key, value[0]]));
    ...
    try {
      ...
      const data = await Promise.all(
        await competitions.map(async (competition) => {
          const participant = await db.collection("applies").countDocuments({ competationsId: competition?._id });
